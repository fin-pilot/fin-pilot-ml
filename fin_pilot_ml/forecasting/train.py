...
from fin_pilot_ml.forecasting.data_loader import FinanceDatasetLoader
from fin_pilot_ml.forecasting.evaluator import ForecastEvaluator
...
def main() -> None:
    ...
    logger.info("Prepared %s observations.", len(series))
    if len(series) < 24:
        raise ValueError("Not enough observations " "for forecasting.")

    test_size = max(
        config.minimum_test_size, int(len(series) * config.test_ratio)
    )

    train_data = series.iloc[:-test_size]
    test_data = series.iloc[-test_size:]

    logger.info("Train size=%s | Test size=%s", len(train_data), len(test_data))
    ...
    hw_model = ForecastModels.train_holt_winters(
        train_data=train_data, seasonal_period=(config.seasonal_period)
    )
    ...
    with open(
        evaluation_dir / "pipeline_metadata.json", "w", encoding="utf-8"
    ) as file:
        json.dump(metadata, file, indent=2, ensure_ascii=False)

    elapsed = time.perf_counter() - start_time
    logger.info("Training pipeline complete " "in %.2f seconds.", elapsed)
